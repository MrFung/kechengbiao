...
- (void)viewDidLoad {
  ...
  _colors = @[@"1b83b4",@"6d9525",@"c58525",@"4161b7",@"af4271",@"7053ab",@"60a779",@"cb5253"];
  [self setModel];
  _addWidth = ([UIScreen mainScreen].bounds.size.width - 30) / 7.0;
  [self setWeekAndDays];
  
  _selectWeekView = [[SelectWeekView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width / 2 - 100, 64, 200, 250)];
  _selectWeekView.backgroundColor = [UIColor colorWithWhite:0 alpha:0];
}
...
- (void)setWeekAndDays {
  ...
  for (NSInteger i = 1; i <= 7; i ++) {
    x --;
    flag = [[weekDay alloc] initWithFrame:CGRectMake(x, 64, _addWidth, height)];
    x += _addWidth;
    flag.alpha = 0.9;
